...
var level = 1;
var krillInitialized = false;

/*
Objects to Krilloud manage 
*/
var canvasObjectSound = 0;
var playerObjectSound = 1;
var enemyObjectSound = 2;
var speedMusic = 0.3;
...
Enemy.prototype.update = async function (dt) {
    ...
    if (player.x >= this.x - 40 && player.x <= this.x + 40) {
        if (player.y >= this.y - 40 && player.y <= this.y + 40) {
            ...
            //* Collision: bugs-sound */
            krilloudSDK.play("bugs", enemyObjectSound);
        }
    }
};
...
Player.prototype.update = async function (dt) {
    ...
    if (this.y < -18) {
        ...
        /* Change speed to background-sound */
        speedMusic++;
        ...
        if (level > 3) {
            ...
            /* Win: win-sound */
            krilloudSDK.stopTag("musica", canvasObjectSound);
            krilloudSDK.stopTag("musicaBugs", enemyObjectSound);
            krilloudSDK.setVar("music_pitch", canvasObjectSound, 0);
            krilloudSDK.play("ui", playerObjectSound);
...
Player.prototype.handleInput = async function (rcv) {
    console.log("RCV", rcv);
    let rcvType = false;
    if (rcv === "left" && this.x > 0) (this.x = this.x - 20), (rcvType = true);
    else if (rcv === "right" && this.x < 400)
        (this.x = this.x + 20), (rcvType = true);
    else if (rcv === "up" && this.y > -50)
        (this.y = this.y - 20), (rcvType = true);
    else if (rcv === "down" && this.y < 400)
        (this.y = this.y + 20), (rcvType = true);

    if (krillInitialized && rcvType) {
        if (this.y >= 280) {
            krilloudSDK.setVar("superficie", playerObjectSound, 0);
            console.log("Play: step grass");
            krilloudSDK.play("pasos", playerObjectSound);
        }

        if (this.y <= 260 && this.y >= 40) {
            krilloudSDK.setVar("superficie", playerObjectSound, 1);
            console.log("Play: step stone");
            krilloudSDK.play("pasos", playerObjectSound);
        }

        if (this.y === 20) {
            krilloudSDK.setVar("superficie", playerObjectSound, 2);
            console.log("Play: step water");
            krilloudSDK.play("pasos", playerObjectSound);
        }
    }

    if (rcv === "left" && this.x > 0) this.x = this.x - 20;
    else if (rcv === "right" && this.x < 400) this.x = this.x + 20;
    else if (rcv === "up" && this.y > -50) this.y = this.y - 20;
    else if (rcv === "down" && this.y < 400) this.y = this.y + 20;
};
...
var showResult = function () {
    $("h3").css("display", "initial").text("¿Start again?");
};
...
$(function () {
    $("canvas").bind("swipeleft", async function () {
        player.handleInput("left");
        krilloudSDK.play("step", playerObjectSound);
    });

    $("canvas").bind("swiperight", async function () {
        player.handleInput("right");
        krilloudSDK.play("step", playerObjectSound);
    });

    $("canvas").bind("swipeup", async function () {
        player.handleInput("up");
        krilloudSDK.play("step", playerObjectSound);
    });

    $("canvas").bind("swipedown", async function () {
...
document.addEventListener("keydown", function (e) {
    /* Play: step player */
    // js_wrapped_EX_Krilloud_Play("step", playerObjectSound)
});
...
var Engine = (function (global) {
    ...
    document.addEventListener("click", async function (e) {
        if (document.getElementById("init").contains(e.target)) {
            reset();
            lastTime = Date.now();
            main();
            document.getElementById("init").style.display = "none";
            document.getElementById("loader").style.display = "none";
            $("h3").css("display", "initial").text("");

            /* Start inicio la música de fondo */
            krilloudSDK.play("musica", canvasObjectSound);
            krilloudSDK.play("musicaBugs", enemyObjectSound);
        }
    });
...
async function krilloud_start() {
    let path = "";
    let relativeWebPath = "/demos/arcade-game";
